...
export default function usePageItems(
  ...
  items: Ref<Item[]>,
  rowsPerPageRef: Ref<number> | Ref<string>,
  ...
  const currentPageFirstIndex = computed((): number =>
    rowsPerPageRef.value === "All"
      ? 1
      : (currentPaginationNumber.value - 1) * (rowsPerPageRef.value as number) +
        1
  );

  const currentPageLastIndex = computed((): number => {
    if (rowsPerPageRef.value === "All") {
      return totalItemsLength.value;
    }
    if (isServerSideMode.value) {
      return Math.min(
        totalItemsLength.value,
        currentPaginationNumber.value * (rowsPerPageRef.value as number)
      );
    }
