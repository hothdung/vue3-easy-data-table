...
export default function useHeaders(
  ...
  const headersForRender = computed((): HeaderForRender[] => {
    ...
    // sorting
    const headersSorting: HeaderForRender[] = fixedHeaders.map(
      (header: HeaderForRender) => {
        ...
        if (serverOptionsComputed.value) {
          if (
            ...
            // single sort
            headerSorting.sortType = serverOptionsComputed.value
  ...
  const updateSortField = (
    ...
    let newSortType: SortType = null;
    if (oldSortType === 'none' || oldSortType === 'desc') {
      newSortType = 'asc';
